...
DynamicSelector::DynamicSelector(const std::string& configFileName, const std::string& selectorName)
  ...
  std::string tagAndProbeConfigFileName = "${LEPTONANALYSIS}/Configuration/LeptonTagAndProbeCuts.cfg";
...
void DynamicSelector::Initialize(Utilities::ObjectManager& objectManager, Utilities::ObjectManager& cutsObjectManager, Cuts::AxisInformation::Expression mode, LeptonAnalysisTree* treeInterface, IO::TreeReader* treeReader, const Binning::Functions::BinningFunction& xAxisDataBinning, const std::string& directoryInFile) {
  ...
  if (mode == Cuts::AxisInformation::Expression::Energy) {
    auto xAxisValue = [treeInterface] (const Analysis::Event&, double& lastBaseValue) {
      if (treeInterface->EcalEnergyBestMaximumShower() != 0)
        lastBaseValue = std::abs(treeInterface->EcalEnergyBestMaximumShower());
    };

    fSelector->SetupCommonXAxisInformation(xAxisValue, "ECAL Energy / GeV", xAxisDataBinning);
  } else if (mode == Cuts::AxisInformation::Expression::Rigidity) {
     auto xAxisValue = [treeInterface] (const Analysis::Event&, double& lastBaseValue) {
      if (treeInterface->TrackerRigidity() != 0)
        lastBaseValue = std::abs(treeInterface->TrackerRigidity());
    };

    fSelector->SetupCommonXAxisInformation(xAxisValue, "Rigidity / GV", xAxisDataBinning);
  }

  auto xAxisValueMc = [treeInterface] (const Analysis::Event&, double& lastBaseValue) {
    if (treeInterface->McGeneratedMomentum() != 0 && treeInterface->IsMcPrimaryGeneratorInAcceptance())
      lastBaseValue = std::abs(treeInterface->McGeneratedMomentum());
  };

  fSelector->SetupCommonXAxisInformationMc(xAxisValueMc, "MC generated momentum / GeV", Binning::Predefined::AbsoluteEnergyBinning);
  ParseAdditionalTagAndProbeInformation(fSelector);

  auto trdBinning = Binning::Equidistant(50, 0.0, 2.5);
  ...
  for (unsigned int cutIndex = 0; cutIndex < cuts; ++cutIndex) {
    ...
    if (auto* efficiencyTemplateFitAttachment = const_cast<Cuts::EfficiencyTemplateFits*>(cut->FindAttachment<Cuts::EfficiencyTemplateFits>())) {
      auto trdValue = [treeInterface]() { return treeInterface->TrdPLRElecProtECAL(); };
      efficiencyTemplateFitAttachment->SetDistributionFunctionAndBinning(trdValue, trdBinning);

      assert(fTagAndProbeRatioFitSettings.find(cut->GetName()) != fTagAndProbeRatioFitSettings.end());
      efficiencyTemplateFitAttachment->SetExcludedTagCuts(fTagAndProbeRatioFitSettings[cut->GetName()].excludedTagCutsForTemplateFit);
    }
  }
}

void DynamicSelector::Initialize(Utilities::ObjectManager& objectManager, Utilities::ObjectManager& cutsObjectManager, Cuts::AxisInformation::Expression mode, LeptonAnalysisTree* treeInterface, IO::TreeReader* treeReader, const std::string& directoryInFile) {

  if (mode == Cuts::AxisInformation::Expression::Energy)
    return Initialize(objectManager, cutsObjectManager, mode, treeInterface, treeReader, Binning::Predefined::AbsoluteEnergyBinning, directoryInFile);

  assert(mode == Cuts::AxisInformation::Expression::Rigidity);
