...
int main(int argc, char** argv) {
  ...
  config.SetProgramHelpText("LithiumPreSelection",
                            "Illustrates the usage of IO::TreeManager to read ROOT trees.");

  config.AddHelpExample("Loop over given ROOT tree.", "--input NucleiAnalysisTree*.root");

  LithiumPreSelection eventLoop;
  eventLoop.ParseOptions(config);

  if (!config.PerformChecksAfterOptionParsing())
    return EXIT_FAIL_CONFIG;

  eventLoop.StartEventLoop("NucleiAnalysisTree", "libNucleiAnalysisLibrary");
